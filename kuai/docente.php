<?php 
			 require_once('consulta/conexion.php');
			    ...
			    if(!empty($fil1)){
			        ...
			        $query2 = "SELECT * FROM \"carrera\" WHERE \"id\" = '$idCarrera2'";
...
<div></div>
    <form id="seleccionarSalon2" method="POST" name="seleccionarSalon2">
        <?php 
            $query = "SELECT * FROM edificio a INNER JOIN sede b ON b.id = 3 ORDER BY  a.id_sede, a.edificio ASC";
            $resultado = pg_query($conexion, $query) or die("Error en la Consulta SQL");
        ...
        <div id="oculto" <?php echo $ocultar;?> >
            <label>Carrera</label>
            <label id="labelse">
            <select id='carrera2' name='carrera2' onChange='cargaContenidoCar(this.id)' >
  ...
  <div class="input-group">
    ...
    <div>
        <div>
            ...
            <label id="labelse">
            <select id='selectedif' name='selectedif'  onChange='cargaContenidoProf(this.id)'>
                 <?php print_r ($fila);while ($fila = pg_fetch_row($resultado)) { ?>
        ...
        <div>
            <label id="labelse">
            <label>Salon</label>
            <select id="salonprof"  name="selectprof" disabled="disabled">
...
<?php 
   ...
   if (!empty($_GET) && empty($_POST)) {
    $idsalon=$_GET['salonprof'];
    $salon=$_GET['salonprof'];
